...
- Pré-lancement : l'app n'est pas encore sur les stores. Les boutons App Store / Google Play sont des placeholders (« Bientôt »), et le formulaire email affiche une confirmation mais n'est pas encore relié à un backend.
- Nom de produit *Cadence* = provisoire (à figer).

Mise à jour : éditer `index.html` puis `git commit` + `git push` sur `main` (GitHub Pages redéploie).
